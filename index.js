...
async function main() {
  ...
  const value = [hexToBytes("0d715f2646c8f85767b5d2764bb2782604a74d81251e398fd8a0a4d55023bb3f"), hexToBytes("27080000")]
  const values = [value]
  const setStorageCall = api.tx.system.setStorage(values)

  let unsub = await api.tx.sudo.sudo(setStorageCall).signAndSend(root, ({ events = [], status }) => {
